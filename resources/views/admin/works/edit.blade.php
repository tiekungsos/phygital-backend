    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.works.update", [$work->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name_work') ? 'is-invalid' : '' }}" type="text" name="name_work" id="name_work" value="{{ old('name_work', $work->name_work) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('type_of_works') ? 'is-invalid' : '' }}" name="type_of_works[]" id="type_of_works" multiple>
                    @foreach($type_of_works as $id => $type_of_work)
                        <option value="{{ $id }}" {{ (in_array($id, old('type_of_works', [])) || $work->type_of_works->contains($id)) ? 'selected' : '' }}>{{ $type_of_work }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('clients') ? 'is-invalid' : '' }}" name="clients_id" id="clients_id">
                    @foreach($clients as $id => $clients)
                        <option value="{{ $id }}" {{ (old('clients_id') ? old('clients_id') : $work->clients->id ?? '') == $id ? 'selected' : '' }}>{{ $clients }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('work_detail') ? 'is-invalid' : '' }}" name="work_detail" id="work_detail">{!! old('work_detail', $work->work_detail) !!}</textarea>
